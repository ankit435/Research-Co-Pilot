...
const Chat = ({ uniqueID, paper = null, aiAssistant = false, aiAgent="pdf_agent"}) => {
  ...
  const { uploadFile,SetaddUploadedFiles, addUploadedFiles } = useAuth();
  ...
  const getStorageKey = (uniqueId, sessionId) => aiAssistant?`chat_${aiAssistant.toString()}_${sessionId}`  :`chat_${uniqueId}_${sessionId}`;
  ...
  const saveToSessionStorageforAI = (messages, sessionId) => {
    if (!aiAssistant || !sessionId) return;

    const key = getStorageKey(aiAssistant.toString(), sessionId);
    sessionStorage.setItem(key, JSON.stringify({
      messages,
      sessionId,
      lastUpdated: new Date().toISOString()
  ...
  const addMessage = (messageData) => {
    setMessages(prev => {
      ...
      if (aiAssistant){
        saveToSessionStorageforAI(newMessages, messageData.session_id);
      }
      else{
    ...
    const connectToAIChat = async () => {
      try {
        const existingSessions = Object.keys(sessionStorage)
          .filter(key => aiAssistant?key.startsWith(`chat_${aiAssistant.toString()}_`)   :key.startsWith(`chat_${uniqueID}_`))
          .map(key => {
